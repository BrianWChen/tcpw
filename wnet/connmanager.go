...
//Add 添加链接
func (connMgr *ConnManager) Add(conn wiface.IConnection) {
    connections := connMgr.connections.Load().(map[uint32]wiface.IConnection)
...
//Remove 删除连接
func (connMgr *ConnManager) Remove(conn wiface.IConnection) {
    connections := connMgr.connections.Load().(map[uint32]wiface.IConnection)
    //删除连接信息
...
//Get 利用ConnID获取链接
func (connMgr *ConnManager) Get(connID uint32) (wiface.IConnection, error) {
    connections := connMgr.connections.Load().(map[uint32]wiface.IConnection)
